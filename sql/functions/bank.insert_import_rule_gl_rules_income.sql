...
-- ============================================================
-- Function : bank.insert_import_rule_gl_rules_income(varchar, varchar,
--              int2, varchar, boolean, varchar, varchar, varchar, varchar,
--              varchar, varchar, varchar, varchar, varchar)
-- ============================================================
-- Purpose  : Creates a complete income import rule, including the rule header,
--            matching criteria, and a GL matrix entry that debits the cash
--            account and credits the income account.
--
-- Parameters
--   s_income_account                   (varchar) : GL description of the income account to credit.
...
--   b_is_deposit                       (boolean) : Deposit/withdrawal filter. Defaults to TRUE.
...
--
-- Usage
--   PERFORM bank.insert_import_rule_gl_rules_income(
--       s_income_account := 'Salary',
--       s_cash_account   := 'ANZ Cheque',
--       s_particulars    := 'SALARY'
--   );
--
-- Dependencies
...
n_income_account_id int4;
n_import_rule_id int;
BEGIN
   
    SELECT account_id INTO n_cash_account_id FROM books.account WHERE description = s_cash_account;
	SELECT account_id INTO n_income_account_id FROM books.account WHERE description = s_income_account;
   
	IF (n_cash_account_id IS NULL OR n_income_account_id IS NULL)
	THEN 
		RAISE EXCEPTION 'unable to insert import rule because cash account %s or income account %s cannot be found', s_cash_account, s_income_account;
		RETURN;
	END IF;
