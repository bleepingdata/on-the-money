DROP FUNCTION IF EXISTS bank.insert_import_rule_gl_rules_assets;

create or replace function bank.insert_import_rule_gl_rules_assets
	(s_cash_account varchar(50),
	s_assets_account varchar(50),
...
n_assets_account_id int4;
n_import_rule_id int;
begin
   
    select account_id into n_cash_account_id from books.account where description = s_cash_account;
	select account_id into n_assets_account_id from books.account where description = s_assets_account;
   
	if (n_cash_account_id is null or n_assets_account_id is null)
	then 
		raise exception 'unable to insert import rule because cash account %s or assets account %s cannot be found', s_cash_account, s_assets_account;
		return;
	end if;

	SELECT bank.insert_import_rule(s_import_rule_type:='assets', n_priority:=n_priority) into n_import_rule_id;
	...
	insert into bank.import_rule_gl_rules_assets (import_rule_id, cash_account_id, assets_account_id)
