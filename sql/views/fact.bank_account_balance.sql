...
(
    ...
    from dimension.dates d
        inner join bank.account a on d.datekey >= a.open_date
        left join books.general_ledger gl on d.datekey = gl.gl_date
            and a.bank_account_id = gl.bank_account_id
    where gl.bank_account_id is null
        and d.datekey >= (
            select min(gl_date)
            from books.general_ledger)
        and d.month_year_date <= (
            select (date_trunc('month', max(gl_date)) + interval '1 month' - interval '1 day')::date
            from books.general_ledger)
    group by a.bank_account_id, d.month_year_date
)
select transactions.bank_account_id,
    transactions.year,
...
from (
    ...
    group by bank_account_id, date_part('year', gl_date), date_part('month', gl_date)
) transactions
order by transactions.bank_account_id,
    transactions.year,
    transactions.month_number;
