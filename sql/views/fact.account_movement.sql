...
select asm.account_id, asm.year, asm.month_number, asm.month_end_date, asm.debit_amount, asm.credit_amount, 
	...
	lag(asm.debit_amount-asm.credit_amount, 1) over (partition by account_id order by year, month_number) as movement_last_month,
	lag(asm.debit_amount-asm.credit_amount,12) over (partition by account_id order by year, month_number) as movement_last_year
