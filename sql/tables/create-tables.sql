...
insert into bank.import_rule_type (import_rule_type_id, description, details)
	values (1, 'Standard', 'A rule which matches the bank''s account to a single other account');
insert into bank.import_rule_type (import_rule_type_id, description, details)
	values (2, 'Fixed Rate Loan Repayment', 'Splits the bank transaction to two accounts. Pays down a payables account to zero, and puts the remainder against the loan principle');

create table bank.import_rule
(
import_rule_id int4 generated by default as identity primary key,
import_rule_type_id smallint not null references bank.import_rule_type(import_rule_type_id) default(1),
bank_account_id int null references bank.account(bank_account_id),
account_id int null references books.account(account_id),
account_id_2 int null references books.account(account_id),
other_party_account_id int not null references books.account(account_id),
other_party_account_id_2 int null references books.account(account_id), -- Optional
